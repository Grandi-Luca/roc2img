# Code for preprocessing ADNI MRI images from https://github.com/SANCHES-Pedro/adni_preprocessing
...
def load_np_image(preprocessed_image_path: Path) -> np.array:
    ...
    if sitk is None:
        raise ImportError(
            "SimpleITK is required for load_np_image; install it with 'pip install SimpleITK'."
        )
...
def main(args: argparse.Namespace):
    ...
    config["re_process"] = args.re_process
    config["axial_size"] = args.axial_size
    config["save_2d"] = args.save_2d
    config["reference_atlas_location"] = args.reference_atlas_location
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ADNI Preprocessing')
    parser.add_argument('--data_path', type=str, required=True, help='Path to the ADNI raw data directory')
    parser.add_argument('--re_process', required=True, help='Reprocess images even if preprocessed files exist')
    parser.add_argument('--axial_size', type=int, required=True, help='Axial size for cropping')
    parser.add_argument('--save_2d', required=True, help='Save preprocessed images as 2D slices')
    parser.add_argument('--reference_atlas_location', type=str, required=True, help='Path to the reference atlas for registration')
